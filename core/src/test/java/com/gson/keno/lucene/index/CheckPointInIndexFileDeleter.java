...
/**
 * https://www.amazingkoala.com.cn/Lucene/Index/2019/1203/113.html
 */
public class CheckPointInIndexFileDeleter {
    ...
    public void doIndex() throws Exception {
        ...
        doc.add(new NumericDocValuesField("age", 0));
        // 这里没有commit，所以不会生成segment2
        oldIndexWriter.addDocument(doc); // A位置
        // 通过NRT获得最新的索引信息，并且没有执行commit()提交
        // openIfChange()方法获得一个包含最新索引信息的reader（即StandardDirectoryReader），
        // 通过该reader获得一个IndexCommit，IndexCommit中包含了A位置
        // 代码增加的索引信息，即以_1为前缀的索引文件，
        ...
        // 在A位置代码，仅仅在添加了一篇文档后，尽管没有执行commit()操作，但是通过NRT获得最新的索引信息后,newIndexWriter仍然有这篇文档的索引数据
